...
import BuffetItemForManager from "@/app/Components/BuffetItemForManager/page";
interface Buffet {
    id: number;
    name: string;
    price: string;
...

            {
                data ? (
                    <div className="grid grid-cols-1 w-[90%] mx-auto mt-8 gap-4 md:grid-cols-2 lg:grid-cols-3">
                        {
                            data.map((item: any) => (
                                <BuffetItemForManager key={item.id} item={item} onChange={handleItemChange} />
                            ))
                        }
                    </div>
                ) : (
...
export default BuffetListManager